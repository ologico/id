...
export async function GET(context: any) {
  ...
  const client = await db.select().from(OAuthClient).where(eq(OAuthClient.id, providerId)).get();
  if (!client) {
  ...
  try {
    ...
    const redirectUrl = new URL(`/app/link-oauth?provider=${providerId}&success=true`, new URL(request.url).origin);
